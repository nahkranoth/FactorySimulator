...
export class DebugController {
    ...
    afterInit(activeChunk) {
        ...
        for (var i = 0; i < 8; i++) {
            let liveNeighboursChunkDebugBounds = new DebugRect({
                scene: this.scene,
                camera: this.camera,
                size: activeChunk.getBounds().width,
                color: 0x0000ff,
                lineColor: 0xff00ff,
                outlinesOnly: true
            });
        ...
        this.activeCameraDebugBounds = new DebugRect({
            scene: this.scene,
            size: 200,
            color: 0xff0000,
            lineColor: 0xff0000,
        ...
        this.activeChunkDebugBounds = new DebugRect({
            scene: this.scene,
            camera: this.camera,
            size: activeChunk.getBounds().width,
            color: 0x0000ff,
