import {SpriteEntityFactory} from "../map/spriteEntityFactory"
...
export class WorldEntityController extends ControllerBaseClass{
    ...
    generateAnimation() {
        var frameNames = this.scene.anims.generateFrameNames('worldEntities', {
            prefix: 'Deer/burn/',
            end: 3,
            zeroPad: 3
        });
    ...
    _addWorldEntity(chunk, x, y, source) {
        ...
        let worldEntity = new worldEntityType({
            x: x,
            y: y,
            frame: source.frame,
            scene: this.scene,
